...
<div id="page-wrapper">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header"><button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#myModal"> Data Pendapatan Harian (Rp) </button></h1>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <div class="panel panel-default">
                    ...
                    <?php 
                      $no = 1;
                      $query = mysqli_query($kon, "SELECT waktu, DATE(waktu) as hari FROM proses GROUP BY hari ORDER BY hari ASC");
                      while($data = mysqli_fetch_array($query)){
                        $hari = $data['hari'];
                        $service = mysqli_fetch_array(mysqli_query($kon, "SELECT SUM(biaya) as total FROM proses WHERE DATE(waktu) = '$hari'"));
                        ?>
                          <tr>
                          <td><?= $no++ ?></td> 
                          <td><?= tgl_indo($hari) ?></td>
